...
class AppTest {
    ...
    // BEGIN
    @Test
    void testReservedSequence1() {
        String test = "array";
        String expected = "yarra";
        CharSequence tempResult = new ReversedSequence(test);
        String result = tempResult.toString();
        assertThat(result).isEqualTo(expected);
        assertThat("r").isEqualTo("" + result.charAt(2));
        assertThat(5).isEqualTo(test.length());
        assertThat("rra").isEqualTo(tempResult.subSequence(2, 5).toString());
    }
    // END
}
